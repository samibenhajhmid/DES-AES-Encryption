    ...
    while i >= 0:
        if print_it:
            if output != '-' and lines_total % 35000 == 0:
                ...
                if lines_total != 0:
                    p(' ETA: %s       ' % sectotime(
                        (time.time() - time_before) / lines_total * (lines_max - lines_total)))
...
def parse(args):
    ...
    while i < len(args):
        ...
        elif args[i] == '-char' or args[i] == '-chars' or args[i] == '--chars' or args[i] == '--char' or args[
            i] == '-c':
            charset += args[i + 1]
            i += 1
        ...
        # resume
        elif args[i] == '-r' or args[i] == '-resume' or args[i] == '--resume' or args[i] == '--r' or args[
            i] == '-res' or args[i] == '--res':
